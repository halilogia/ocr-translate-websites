  ...
  static async performOCR(
    ...
    if (!model) {
      throw {
        ...
        details:
          "Please select a vision model in Settings (e.g., llava, moondream). You can install one using: ollama pull llava",
      } as OCRError;
    }
    ...
    try {
      ...
      const response = await fetch("/api/ocr", {
        ...
        body: JSON.stringify({
          image: imageSrc,
          model: model,
          prompt: "Read all text from this image exactly as written. Include all speech bubbles and sound effects. Output only the text, nothing else.",
        }),
      });
      ...
      console.log(
        `[OllamaEngine] Response text length: ${result.text?.length || 0}`,
      );
