    ...
    try {
      ...
      const response = await fetch("/api/ocr", {
        ...
        body: JSON.stringify({
          ...
          model: model,
          prompt:
            "Extract all visible text from this image. Output ONLY the raw text found, maintaining original line breaks. Do not add descriptions, headers, or explanations. If text is unclear, output what you can read.",
        }),
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Aether Failure (${response.status}): ${errData.error || "Unknown"}`,
        );
      }
